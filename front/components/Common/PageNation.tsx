...
const PageNation = () => {
    ...
    const switchPage = (next: boolean) => {
        ...
        if (next){
            ...
            for (let i = currentStep; i < currentStep + 5; i++) {
                if(i <= 0)continue
                steps.push(i)
    ...
    return (
        <div className={"flex items-center justify-center shadow-xl"}>
            <div onClick={() => switchPage(false)} className={"rounded-l-full cursor-pointer border-y border-l py-2 w-[60px] text-center mx-auto"}>
            ...
            {pages.map((page, index) => (
                <div
                    ...
                    className={currentStep === page ? "w-[60px] cursor-pointer text-white bg-[rgb(125,24,20)] border-y border-l py-1" : "w-[60px] cursor-pointer text-[rgb(125,24,20)] border-y border-l py-1"}
                    key={index}
                >
                    {page}
                </div>
            ))}
            <div onClick={() => switchPage(true)} className={"rounded-r-full border cursor-pointer py-2 w-[60px] text-center mx-auto"}>
